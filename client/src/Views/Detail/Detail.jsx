    ...
    <div className="py-6 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center space-x-2 text-gray-400 text-sm">
          <Link to="/" className="hover:underline hover:text-gray-600">Inicio</Link>
          <span>
        ...
        <div className="flex flex-col md:flex-row -mx-4">
          <div className="md:flex-1 px-4">
            <div className={`h-64 md:h-80 rounded-lg bg-gray-100 mb-4 flex items-center justify-center`}>
              {detail.imagen ? (
                <img src={detail.imagen[image - 1]} alt={`Imagen ${image}`} className="w-full h-full object-cover" />
              ) : (
                <img src={images[image - 1]} alt={`Imagen ${image}`} className="w-full h-full object-cover" />
              )}
            </div>
              ...
              {(detail.imagen || images).map((img, i) => (
                <div key={i} className="flex-1 px-2">
                  <button onClick={() => changeImage(i + 1)} className={`focus:outline-none w-full rounded-lg h-24 md:h-32 bg-gray-100 flex items-center justify-center ${image === i + 1 ? 'ring-2 ring-indigo-300 ring-inset' : ''}`}>
                    <img src={img} alt={`Miniatura ${i + 1}`} className="w-full h-full object-cover rounded-lg" />
                  </button>
                </div>
              ))}
            </div>
          ...
          <div className="md:flex-1 px-4">
            ...
            <p className="text-gray-500 text-sm">Por <a href="#" className="text-red-500 hover:underline">{detail?.marca}</a></p>
            ...
            <div className="flex items-center space-x-4 my-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-400 text-sm">Incluye todos los impuestos.</p>
              </div>
            </div>
            ...
            <div className="flex py-4 space-x-4">
              <div className="relative">
                ...
                ? ( Object.entries(detail.stock).filter(([talle, cantidad]) => cantidad > 0).length > 0 

                     ? (Object.entries(detail.stock).filter(([talle, cantidad]) => cantidad > 0)
                     .map(([talle]) => (<option key={detail.stock[1]} className="text-center">{talle}</option>))) 
